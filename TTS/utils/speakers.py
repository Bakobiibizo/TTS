    ...
    if c.use_speaker_embedding:
        ...
        if args.restore_path:
            if c.use_external_speaker_embedding_file: # if restore checkpoint and use External Embedding file
                ...
                assert all(
                    speaker in speaker_mapping for speaker in speakers
                ), (
                    "As of now you, you cannot "
            ...
            speaker_mapping = load_speaker_mapping(c.external_speaker_embedding_file)
            speaker_embedding_dim = len(speaker_mapping[list(speaker_mapping.keys())[0]]['embedding'])
        elif c.use_external_speaker_embedding_file: # if start new train using External Embedding file and don't pass external embedding file
        ...
        print(f' > Training with {len(speakers)} speakers: {", ".join(speakers)}')
    else:
        num_speakers = 0
        speaker_embedding_dim = None
